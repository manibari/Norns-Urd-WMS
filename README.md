...
收貨就必須留下到期依據 —— 標示的有效日期，或製造日期加上保存天數。沒有任一項會被擋下來，
否則效期提醒對那批會安靜地永遠不觸發。
...
- **型號的屬性**（廠商／原物料名稱／規格）→ 直接讀型號表的 distinct 值。它們是型號的欄位，
  另開一張字典等於同一件事存兩份，遲早對不起來。要改就編輯那個型號。
- **獨立選項**（職位／機台／產品名稱／覆核原因）→ `dictionary` 表（category + value + 啟用狀態）。
