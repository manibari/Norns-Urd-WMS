...
export default function RecordsPage() {
  ...
  const load = useCallback(async () => {
    ...
    const [scans, d] = await Promise.allSettled([api.scans(), api.dictionary()]);
  ...
  const usageTab = (
    <>
        <Space size={24} style={{ marginBottom: 16 }}>
          ...
          {pending > 0 && <Text type="warning">{pending} 筆產品名稱待補</Text>}
        </Space>
        <Table
          rowKey="id"
          dataSource={rows}
          ...
          columns={[
            ...
            { title: "產品名稱", render: (_, row) => row.field_values["產品名稱"] || <Text type="secondary">待補</Text> },
            {
              title: "",
              width: 100,
              render: (_, row) =>
  ...
  return (
    <>
      ...
      <Card title={`領用紀錄（${rows.length}）`}>{usageTab}</Card>
