...
export default function ReceivingPage() {
  ...
  // The form's 數量 column is whatever that item is counted in — metres for
  // film, sheets for foil bags, packs for desiccant — so the toggle says
  // "單上數量" rather than naming a unit that would be wrong half the time.
  ...
  const picked = items.find((i) => i.id === chosenItem);
  const chosenLabel = picked?.label;
  // The item decides whether an expiry date is required — 肉乾真空膜 needs none,
  // 脫氧劑 does (form note 1). Marking it required only for those items keeps
  // the asterisk meaningful.
  ...
  const preview =
    qtyMode === "單上數量" && rate && rate > 0 && metres && metres > 0
    ...
    <>
      ...
      >
        ...
        >
                ...
                <Select
                  ...
                  options={items.map((i) => ({
                    ...
                    label: `${i.label}｜${i.name}${i.spec ? ` ${i.spec}` : ""}`
                      + `${i.meters_per_box ? `（每箱 ${i.meters_per_box}${i.pack_unit ?? ""}）` : ""}`,
                  }))}
                />
              </Form.Item>
            </Col>
            ...
            <Col xs={24} md={6}>
              <Form.Item
                ...
                extra={
                  qtyMode === "單上數量"
                    ? preview
                      ? `= ${preview.boxes} 箱${preview.remainder ? `，剩 ${preview.remainder.toLocaleString()}${picked?.pack_unit ?? ""} 對不上整箱` : ""}`
                      : rate
                        ? `每箱 ${rate.toLocaleString()}，輸入單上數量自動換算`
                        : chosenItem
                          ? `${chosenLabel ?? "此品項"} 尚未設定每箱數量`
                          : "先選品項"
                    : undefined
                }
                ...
                extra={
                  needsExpiry
                    ? picked?.shelf_life_days
                      ? `${chosenLabel} 有保存期限：填有效日期，或填製造日期讓系統以 ${picked.shelf_life_days} 天推算`
                      : `${chosenLabel} 有保存期限，必須填有效日期`
                    : "多數包材沒標，留空是正常的"
                }
              >
                <DatePicker style={{ width: "100%" }} />
          ...
          {qtyMode === "單上數量" && Boolean(chosenItem) && !rate && (
            <Alert
              type="warning"
              title="要用單上數量收貨，得先設定每箱數量"
              description="就填在下面「每箱數量」，或到「基本資料」統一維護。庫存單位一律是箱 —— 一箱是一個完整包裝，不做部分入庫。"
              style={{ marginBottom: 16 }}
            />
          )}
